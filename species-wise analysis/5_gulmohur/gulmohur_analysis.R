...
fl_climwin_mod3 <- phenWin3[[3]]$BestModel
summary(fl_climwin_mod3)
moddat <- phenWin3[[3]]$BestModelData
phenOutput <- phenWin3[[3]]$Dataset
plot(moddat$yvar~moddat$climate)
plotdelta(phenOutput)
plotweights(phenOutput)
plotwin(phenOutput)
...
phenWin4 <- slidingwin(xvar = list(rain = rain, temp = temp),
                       cdate = wdat$Date1,
                       bdate = dat_phenf$datep1,
                       baseline = glm(prop ~ 1, data = dat_phenf, family = 'binomial'),
                       cinterval = 'day',
...
fl_climwin_mod4 <- phenWin4[[2]]$BestModel
summary(fl_climwin_mod4)
moddat <- phenWin4[[2]]$BestModelData
phenOutput <- phenWin4[[2]]$Dataset
plot(moddat$yvar~moddat$climate)
plotdelta(phenOutput)
plotweights(phenOutput)
plotwin(phenOutput)
