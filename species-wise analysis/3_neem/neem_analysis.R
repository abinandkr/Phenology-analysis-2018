...
fl_mod3 <- glm(prop~photoperiod + diff2,data = dat_phenf,family = 'binomial')
summary(fl_mod3)
plot_model(dat_phenf$prop,fl_mod3)

fl_mod4 <- glm(prop~temp + diff2,data = dat_phenf,family = 'binomial')
summary(fl_mod4)
plot_model(dat_phenf$prop,fl_mod4)
...
phenWin2 <- slidingwin(xvar = list(rain = rain, temp = temp),
                       cdate = wdat$Date1,
                       bdate = dat_phenf$datep1,
                       baseline = glm(prop ~ 1 + diff2, data = dat_phenf, family = 'binomial'),
                       cinterval = 'week',
                       range = c(47,0),
                       type = 'relative',
                       stat = c('mean'),
...
fl_climwin_mod2 <- phenWin2[[1]]$BestModel
...
phenWin3 <- slidingwin(xvar = list(temp = temp),
                       cdate = wdat$Date1,
                       bdate = dat_phenf$datep1,
                       baseline = glm(prop ~ 1 + diff + photoperiod, data = dat_phenf, family = 'binomial'),
                       cinterval = 'week',
                       range = c(40,0),
                       type = 'relative',
                       stat = c('mean'),
