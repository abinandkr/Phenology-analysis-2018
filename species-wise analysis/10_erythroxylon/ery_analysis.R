...
dat_spc <- dat %>% filter(species_id == 10)
rm(dat)
...
dat_phen$datep <- format(as.Date(dat_phen$datep), '%d/%m/%Y')
phenWin <- slidingwin(xvar = list(rain = rain, temp = temp),
                      cdate = wdat$Date,
                      bdate = dat_phen$datep,
                      baseline = glm(prop ~ 1, data = dat_phen, family = 'binomial'),
                      cinterval = 'day',
...
dat_phen <- dat_spc1 %>% filter(phenophase == 'leaves_new', datep < '2014-01-01',datep > '2008-12-31')
...
testdat <- dat_spc_phen %>% filter(ind_id %in% c(22,38))


#######################

dat_spc_phen <- dat_spc_phen %>% filter(!is.na(value))
dat_spc_phen$datep <- format(as.Date(dat_spc_phen$datep), '%d/%m/%Y')
phenWin <- slidingwin(xvar = list(rain = rain, temp = temp),
                      cdate = wdat$Date,
                      bdate = dat_spc_phen$datep,
                      baseline = glmer(value ~ 1 +(1|ind_id), data = dat_spc_phen, family = 'binomial'),
                      cinterval = 'week',
                      range = c(20,0),
                      type = 'relative',
                      stat = c('mean','max'),
